...
import { createCarCatalog, type CarCatalogCreateData } from '../api'
...
export default function AddCarCatalogPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const catalogData: CarCatalogCreateData = {
      ...
      const response = await createCarCatalog(catalogData)

      if (response.success) {
        toast.success('Car catalog item created successfully!')
        router.push('/dashboard/car-catalog')
      } else {
        toast.error('Failed to create car catalog item')
      }
    } catch (error: any) {
      console.error('Create catalog error:', error)
      toast.error(error.response?.data?.message || 'An error occurred while creating the catalog item')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Add Car Template</h1>
          <p className="text-gray-600 mt-2">
            Create a new car template for the catalog
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Fill in the details to create a new car template
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-4">
              <Button 
                ...
                className="flex-1"
              >
                {loading ? 'Creating...' : 'Create Car Template'}
              </Button>
              <Button
                type="button"
                variant="outline"
